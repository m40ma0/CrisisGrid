  ...
  return (
    <section className="rounded-lg border border-command-line bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h2 className="text-sm font-bold text-zinc-950">Command Briefing</h2>
          <p className="text-xs text-zinc-500">
            {apiStatus.gemini === "fallback" ? "Fallback analyst text" : "Gemini explanation layer"}
          </p>
        </div>
      ...
      {isGeneratingBriefing ? (
        ...
        <div className="rounded border border-dashed border-zinc-300 bg-zinc-50 p-4 text-sm text-zinc-500">
          The briefing will summarize the generated plan.
        </div>
      )}
    </section>
  );
